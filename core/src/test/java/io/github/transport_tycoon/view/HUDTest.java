...
public class HUDTest {
    ...
    @Test
    public void testUpdateBalanceAndShowChange() {
        hud.updateBalance(5000f);
        hud.showBalanceChange(500f);
        hud.showBalanceChange(-200f);
        assertNotNull(hud.getStage());
    }

    @Test
    public void testUpdateTime() {
        hud.updateTime("Day 5");
        assertNotNull(hud.getStage());
    }

    @Test
    public void testSetBuildModes() {
        hud.setBuildModeActive(true);
        hud.setBuildModeActive(false);
        hud.setStopBuildModeActive(true);
        hud.setStopBuildModeActive(false);
        assertNotNull(hud.getStage());
    }

    @Test
    public void testUpdateTooltipCity() {
        when(mockCity.getName()).thenReturn("Testville");
        when(mockCity.getGridWidth()).thenReturn(2);
        when(mockCity.getGridHeight()).thenReturn(2);
        when(mockCity.getDemands()).thenReturn(new HashMap<>());

        hud.updateTooltip(mockCity, 500, 500);
        assertNotNull(hud.getStage());
    }

    @Test
    public void testUpdateTooltipFacility() {
        when(mockFacility.getFacilityType()).thenReturn("Lumber Mill");
        when(mockFacility.getGridWidth()).thenReturn(2);
        when(mockFacility.getGridHeight()).thenReturn(2);
        when(mockFacility.getProduces()).thenReturn(GoodType.WOOD);
        when(mockFacility.getConsumes()).thenReturn(null);

        hud.updateTooltip(mockFacility, 500, 500);
        assertNotNull(hud.getStage());
    }

    @Test
    public void testClearTooltip() {
        hud.updateTooltip(null, 0, 0);
        assertNotNull(hud.getStage());
    }

    @Test
    public void testRenderAndResize() {
        hud.render();
        hud.resize(1280, 720);
        assertNotNull(hud.getStage());
    }
